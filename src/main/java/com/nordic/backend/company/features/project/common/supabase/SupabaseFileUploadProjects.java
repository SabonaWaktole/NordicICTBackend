package com.nordic.backend.company.features.project.common.supabase;
...
@Component
public class SupabaseFileUploadProjects {
    ...
    public SupabaseFileUploadProjects(@Value("${supabase.url}") String supabaseUrl) {
    ...
    public String uploadFile(MultipartFile file) {
        try {
            ...
            //TODO : Change bucket name and filepath accordingly

            // Use UriComponentsBuilder for safe URL construction
            URI uploadUri;
            try {
                uploadUri = UriComponentsBuilder.fromHttpUrl(supabaseUrl)
                        .path("/storage/v1/object/ProjectsPhoto/"+fileName)
                        .queryParam("bucket", "ProjectsPhoto")
                        .queryParam("name", fileName)
                        .queryParam("bucketId", "ProjectsPhoto")
                        .build()
                        .toUri();
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                return baseUri.resolve("/storage/v1/object/public/ProjectsPhoto/" + fileName).toString();
            } else {
