package com.nordic.backend.company.features.news.common.supabase;
...
@Component
public class SupabaseFileUploadNews {
    ...
    public SupabaseFileUploadNews(@Value("${supabase.url}") String supabaseUrl) {
        ...
        try {
            ...
            try {
                uploadUri = UriComponentsBuilder.fromHttpUrl(supabaseUrl)
                        .path("/storage/v1/object/NewsPhotos/"+fileName)
                        .queryParam("bucket", "NewsPhotos")
                        .queryParam("name", fileName)
                        .queryParam("bucketId", "NewsPhotos")
                        .build()
                        .toUri();
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                return baseUri.resolve("/storage/v1/object/public/NewsPhotos/" + fileName).toString();
            } else {
